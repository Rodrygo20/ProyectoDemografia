...
ggplot(lt_output %>% as_tibble(), aes(x = age, y = log(mx), color = factor(year), group = year)) +
  ...
  theme_minimal()
# Pégalo al final de la gráfica de log(mx)
ggsave("evolucion_mortalidad_mx_sinaloa.png", width = 8, height = 5, dpi = 300)
```
```{r}
...
ggplot(lt_output_plot, aes(x = age, y = ex, color = factor(year), group = year)) +
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
# Pégalo al final de la gráfica de líneas de ex
